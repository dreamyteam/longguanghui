...
var gBegin=[2003,1,1];	// calendar date range begin from [Year,Month,Date]. Using gToday here will make it start from today.
var gEnd=[2010,12,31];	// calendar date range end at [Year,Month,Date]
var gsOutOfRange="对不起,您不能超出日期的范围!";	// out-of-date-range error message. If set to "", no alerts will popup on such error.
...
var gsBottom="<A href='javascript:void(0)' class='Today' onclick='if(this.blur)this.blur();if(!fSetDate(gToday[0],gToday[1],gToday[2]))alert(\"Today is not a selectable date!\");return false;' onmouseover='return true;' title='今天'>今天 : "+gToday[0]+"年"+gMonths[gToday[1]-1]+gToday[2]+"日</A>";	// the content of the bottom section.
