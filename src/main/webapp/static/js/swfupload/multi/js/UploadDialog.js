...
UploadDialog = Ext.extend(Ext.Window, {
	...
	constructor : function(config) {
		...
		this.fileList = new Ext.grid.GridPanel({
			...
			columns : [new Ext.grid.RowNumberer(),
		        ...
		        {header: '状态', width : 50, dataIndex : 'fileState', renderer : this.formatState, sortable:false, fixed : true, align:'center' },
		        {header: '消息', width : 350, dataIndex : 'fileMsg', renderer : this.formatState, sortable:false, fixed : true, align:'center' }
			],
			autoExpandColumn : autoExpandColumnId,
			ds : new Ext.data.SimpleStore({fields: ['fileId','fileName', 'fileSize','fileType','fileState', 'fileMsg']}),
	...
	formatProgress : function(info) {
		...
		var v = Math.ceil(p * 100);
		v = v >= 100 ? 100 : v;
		this.progressBar.updateProgress(p, "上传中 " + v + " %");
		return r;
	},
	...
	formatState : function(_v, cellmeta, record){
		...
		switch(_v){
			...
			default : 
				if(_v != ''){
					returnValue = '<span id="' + record.id + '">' + _v + '</span>';
				}
				break;
		}
		return returnValue;
	...
	onFileQueued : function(file){
        ...
        if(fileNumb && thiz.progressInfo.filesTotal >= fileNumb){
        	//超过文件数
        	alert("文件已满!");
        	return;
        }
	...
	onUploadComplete : function(file){
		if(!fileNumb || this.customSettings.scope_handler.progressInfo.filesUploaded<fileNumb){
