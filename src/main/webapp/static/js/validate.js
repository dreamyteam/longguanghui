     ...
     if(frm.elements[i].alt=='key'){
	     if(trim(frm.elements[i].value)==''){ 
	    	 if(frm.elements[i].type=='text'){
	    		addFieldError("请输入"+frm.elements[i].title);
			 	frm.elements[i].select();
			}else{
				addFieldError("请选择"+frm.elements[i].title);
...
function checkPlusInt(elem,field){
	...
	if(pattern.test(val) || val.slice(0,1)=="0"){
		addFieldError(field+"只能输入非零的整数！",elem);
		elem.value = '';
		return false;
	}
	return true;
...
function checkInteger(elem,field){
	...
	var pattern=/[^0-9]/;
	if(pattern.test(val)){
		addFieldError(field+"只能输入非负整数！",elem);
		return false;
	}
	return true;
}
...
function checkNum(elem,field){
	...
	var pattern=/[^0-9.]/;
	if(pattern.test(val)){
		addFieldError(field+"只能输入非负数值型数据！");
		elem.value = '';
		return false;
	}
	return true;
...
function checkNumRange(elem,field,min,max){
	...
	if(val<=min || val>=max){
		addFieldError(field+"只能是"+min+"~"+max+"范围内的数值！");
		return false;
	}
	return true;
}
...
function checkOverLen(elem,maxlen,field){
	...
	if(len>maxlength){
		addFieldError(field+"长度超过范围,允许最大长度为 "+maxlength+" 字符(一个汉字占2个字符)");
		return false;
	}
	return true;
}
		 ...
		 for(var i=0;i<date1.length;i++){
			if(date1[i]>date2[i]){
				if(elem1.getAttribute("type") !='hidden'){
					addFieldError(elem1.getAttribute("msgName") + "与" + elem2.getAttribute("msgName") + "之间 前后时间不合逻辑，请更正！");
...
function checkDate_(elem,field){
    ...
    if (err!= 0) {
    	addFieldError( '要求'+ field + '的格式为mm-dd，并请注意月份与日期的有效范围');   
	return false;
    }
    return true;
}
...
//    		alert(field + '的值不能小于1900-1-1');			
			addFieldError(field + "的值必须在1900-1-1到2038-12-31之间");			
    		elem.select();
			return false;
    	}
...
//    		alert(field + '的值不能大于2038-12-31');
    	...
    	addFieldError('要求'+ field + '的格式为yyyy-mm-dd，并请注意月份与日期的有效范围');   
//        alert(field + '为日期型\n日期的格式为yyyy-mm-dd\n并请注意月份与日期的有效范围');    
        elem.select();
		return false;
    }
...
function checkIdentityCard(elem,showPrompt){	
    ...
    if(reOld.test(elem.value)) {
    	...
    	if(showPrompt == undefined || showPrompt)
        	addFieldError('请输入正确的身份证号！', elem.name);
        return false;
    }
...
function checkMobilePhone(elem){
	...
	if(trim(val)!=''){		
		...
		//alert(val);
		if(!pattern.test(val)){
			addFieldError("请输入正确的号码，并且不能超过20个数字！");
			return false;
		}
	}
	return true;
...
function checkipport(elem,field){
	...
	if(!pattern.test(val)){
		addFieldError(field+'请确认符合http://IP:PORT格式');
		return false;
	}
	return true;
}
...
function checkPhone(elem,field){
	...
	if(val.indexOf('-')==-1){
		...
		if(!pattern.test(val)){
			addFieldError('请输入正确的'+field+",并且不能超过12个数字");
			return false;
		}
		return true;		
	}
...
function checkEmail(elem){	
    ...
    if(reEmail.test(elem.value)) {
        ...
        addFieldError('Email格式不对，请输入正确的Email地址！');
        return false;
    }
}
...
function checkPostalCode(elem){	
    ...
    if(pattern.test(elem.value)) {
        return true;
   	 	...
   	 	addFieldError('邮政编码格式不对，请输入正确的邮政编码！');
        return false;
    }
}
...
function checkwebaddress(elem,field){
	...
	if(!pattern.test(val.substring(0,7))){
		addFieldError(field+'请确认符合http://IP格式或http://域名格式');
		return false;
	}
	return true;
}
...
function checkInvalidChar(obj){
	...
	if(pattern.test(obj.value)){
		addFieldErrorWithObject("不能有<>~!@#$%&|等字符");
		return false;
	}
	return true;
}
...
function checkQuery(elem, elemName){
	...
	if (p.test(elem.value)){
		addFieldError(elemName + "内容不能包含以下任何字符: '%_^|/][*");	
		return false;
	}
	return true;
}
 ...
 * @return
 */
function checkAllValidate(containerName){
			...
			for(var i = 0; i < os.length; i ++){
				...
				if(o.value.trim().length > 0){
					...
					if(maxLength && maxLength > 0 && maxLength < 100000){
						var valueLength = getLength(o.value);
						if(valueLength > maxLength){
							addFieldError(osName + " 长度为" + valueLength + "个字符，超出了最大长度限制：" + maxLength + "个字符");
							return false;
						}
					}
						...
						if(ch){
							...
							if(max){
								if(o.value > parseFloat(max)){
									addFieldError(osName + " 值不能超过" + max);
									return false;
								}
							}
							if(min){
								if(o.value < parseFloat(min)){
									addFieldError(osName + " 值不能小于" + min);
									return false;
								}
							}
							if(integerLength){
								...
								if(v.length > integerLength){
									if (dl){
										addFieldError(osName + " 输入格式不对，要求是不超过" + integerLength + "位整数，" + dl + "位小数的数值");
										return false;
									}
									addFieldError(osName + " 输入格式不对，要求是不超过" + integerLength + "位整数位的数值");
									return false;
								}
							}
							if(nonnegative){
								if(nonnegative && (nonnegative.toLowerCase() == "yes" || nonnegative == "1" || nonnegative.toLowerCase() == "true" )){
									if(o.value.indexOf("-")>=0){
										addFieldError(osName + " 输入格式不对，只能输入非负数值型数据");
										return false;
									}
								}
							}
							...
							if (dl)
								addFieldError(osName + " 输入格式不对，要求是不超过" + dl + "位小数的数值");
							else
								addFieldError(osName + " 输入格式不对");
							return false;
						}
					}
				}
